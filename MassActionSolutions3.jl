...
# ╔═╡ 821cf9c7-b9e4-40cc-ae52-c9541934695d
u₀=[0.02,0.01,0];
...
md"Next, we rename each row as the corresponding variable and plot the results in a graph, with the x-axis as the time vector `tspan`, and the y-axis the concentration of each variable `a,b,c`:"

# ╔═╡ 783856d0-3105-492b-ac64-9b261d321b61
a=U[1,:];

# ╔═╡ 198b1bf5-7761-4c2c-a3cb-a36d9b6dbea0
b=U[2,:];

# ╔═╡ 1bdb8e1e-3bd0-4185-8c52-a9c43bb68958
c=U[3,:];
...
begin
	...
	v₀2=1.0
	v₀=[v₀1,v₀2];
	V=zeros((size(v₀,1), size(tspan,1)));
	V[:,1]=v₀;
end

# ╔═╡ ac273181-fb9e-42af-9cc5-4d7af20e48ca
for i = 2:size(tspan,1)
           V[1,i]= V[1,i-1]-k1*2*V[1,i-1]*V[1,i-1] + 2* k2* V[2,i-1] # Eq 1
           V[2,i]= V[2,i-1]+k1*V[1,i-1]*V[1,i-1] - k2* V[2,i-1] # Eq 2
       end
...
begin
	aa=V[1,:]
	bb=V[2,:]
	plot(tspan,aa,label="a",seriestype=:scatter)
	plot!(tspan,bb,label="b",seriestype=:scatter)
	title!("Euler in Julia")
	xlabel!("Time [s]")
	ylabel!("Concentration [M]")
end
...
md"We can use now the single function that comes from the Masa Conservation Law"
...
begin
	...
	W[1]=w₀	
	for i = 2:size(tspan,1)
           W[i]= W[i-1] - 2*k1*W[i-1]*W[i-1] + k2 * (v₀1+  2* v₀2 -W[i-1]) # Eq 1
    end
	plot(tspan,W',label="a",seriestype=:scatter)
	plot!(tspan, v₀2 .+ v₀1 ./2  .- W' ./2,label="b",seriestype=:scatter)
	title!("Euler in Julia")
	xlabel!("Time [s]")
	ylabel!("Concentration [M]")
end
...
# ╔═╡ dcb48864-a4cd-4153-88a8-ad3d95dcc9aa
...
# ╔═╡ 01344912-3646-4382-9335-8e3fef001863
md"It is not an exponential, is a mix of a 1/x and an exponential "

# ╔═╡ 7a184872-cc95-47df-b654-983985805eb4
