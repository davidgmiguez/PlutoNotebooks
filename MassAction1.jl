...
md"# Basics of Chemical Dynamics
...
In other words, equilibrium occurs when the concentration of the reactants do not change overtime. In our example, it means that you reach a value of $[N_{2}O_4]$ and $[NO_2]^2$ that is constant. As a consequence, in reversible reactions at equilibrium, the ratio $\frac{[N_{2}O_4]}{[NO_2]^2}$ is constant and, looking at eq 21, this ratio is proportional to the ratios between the reaction rates $k_1$ and $k_2$. 

Another direct consequence, is that the ratio between the concentrations of reactants at equilibrium ($\frac{[N_{2}O_4]}{[NO_2]^2}$ in our example) does not depend on how much $[N_{2}O_4]$ or $[NO_2]^2$ you put intially in the system. Only depends on the ratio between the rates of the two reversible equations. 

On the other hand, it does depend on the temperature, because the reactions rates depend linearly on the temperature. In brief, since the velocity of the particles depends on the temperature so number of ''succesfull'' collisions also depens on the temperature.  This dependence is linear and is given by the Van't Hoff equation. Also, this linear dependence on the temperature is often different for different reactions, so each $k$ will change differently when the temperature changes. 
"
...
md"""  We can directly see in this type of energy diagrams that the $E_a$ represents the energy barrier of the transition state. In this case, the plot  represents an endothermic reaction (energy of products is higher than energy of reactans, and a supply of energy in the form of temperature is required, which usually means that will be taken from the surroundings as the reaction takes place). 
...
md"""Another interesting graph is the progress in reaction energy for exotermic reactions (reactions that release heat, as the reaction takes place). Despite being energetically favorable, we also have a Activation energy, $(Resource(exothermic_url))""" 
...
begin
	p5=plot(T,log(A).-(E_a./2 ./ (0.082.*T)),ylims = (-2,0),label = "k1, Exothermic")
	p5=plot!(T,log(A).-(E_a./(0.082.*T)),ylims = (-2,0),label = "k2, Endothermic")
...
md"As as general conclusion, changes in the temperature of the system that result in an increase in $K_{eq}$ shift the equilibrium towards the products, while a reduction in $K_{eq}$ shifts the concentrations at equilibrium favoring the reactants.  
"
...
# ╔═╡ eea0009f-5907-4ba9-9d5d-56b4fd0d96dd
NaCO_3=2;

# ╔═╡ 2a8666a9-64a7-433b-a1b5-8248f6b3e79b
CaCl_2=0.5;

# ╔═╡ 4372ab8a-d81b-4cd7-aa39-37d4670a9b88
CaCO_3=2;

# ╔═╡ 4472436b-58bf-4cbc-8055-9164577cc93b
NaCl=1.2;

# ╔═╡ 4e711021-eddf-4cb2-a420-e6f17dad5542
md"Then the equilibrium constant can be calculated as:"

# ╔═╡ 27f47b62-38c9-4f64-aae0-0d7c216ac05e
(CaCO_3*NaCl^2)/(NaCO_3*CaCl_2)
...
As a first task, write a simple computer program that consists on a single function that takes as input arguments, a vector of concentrations at equilibirum of the  reaction 30 and returns the value $K_{eq}$"

# ╔═╡ 43d9782d-6829-4d51-bcae-4d70a8596fb2
function Calculate_Keq(NaCO_3,CaCl_2,CaCO_3,NaCl)
	CaCO_3*NaCl^2/NaCO_3*CaCl_2
end

# ╔═╡ 2aba6b28-a931-489e-aad2-90411ec61ede
Calculate_Keq(NaCO_3,CaCl_2,CaCO_3,NaCl)
