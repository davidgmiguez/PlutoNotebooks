...
# ╔═╡ b57ade87-40af-4f58-b56d-7f883d5b2367
html"<button onclick='present()'>present</button>"

# ╔═╡ 336cae68-a9fd-45a0-8ba8-0fd6fab4a287
begin
	struct Foldable{C}
	    title::String
	    content::C
...
md"# 8. The Mass Action Law

## 8.1 Definition of the Law of Mass Action 

Now that we know how to obtain the stoichiometric matrices and the order of any given reaction, the next step is to write the differential equations that capture the dynamics, i.e., the changes in time of the amount of all the species involved.  
##
...
# ╔═╡ 7d856c88-f52b-497d-b64c-281da39eff6f
A1=[1 1 0]

# ╔═╡ 30a79e11-2165-4908-94a3-d020cc6d2e16
md"The order of the reaction is $(sum(A1))"

# ╔═╡ e11754b1-6993-403b-a0ec-e0aaf86a5dd3
md"The Units of k are s^-1 x k^$(1-(sum(A1)))"
...
##
The __Mass Action Law__ describes how a chemical reaction takes place under ideal and constant conditions. In brief, it is based on the fact that the speed of any reaction is proportional to the concentrations of the reactants involved, which has been discussed previously in this course (the proportionalinty constant is the kinetic rate constant). 
##
From here, it follows that the rate of change of any reactant is also proportional to the concetrations of the reactants involved. This velocity depends on the affinity of the reaction and the concentrations of each of the reactants (in any scheme of interactions, the ones at the left of the arrow). 
##
The `k` is the proportionality constant that correlates the speed fo the reaction with the concentrations of the species involved, which in this case are $X_1$ and $X_2$:
...
# ╔═╡ 4f218166-40d1-4ab0-adf4-e938061bf78d
A2=[2 1 0]

# ╔═╡ 0e326b5a-c8df-4c28-9ffb-4581d6dac0cb
B2= [0 0 2]

# ╔═╡ fee636f1-4944-46a4-bcbe-32b7273268c7
md"The order of the reaction is $(sum(A2))"

# ╔═╡ 56a68cf6-6e64-4875-b614-18987857bc06
md"The Units of k are s^-1 x k^$(1-(sum(A2)))"
...
We will write down the differential equations that account for the dynamics of each species. The speed of each reaction depends on the concentration of the species involved (what is before the arrow). In this case is two units of $X_1$ and one unit of $X_2$. 
##
In addition, since the reaction consumes 2 units of $X_1$ for each unit of $X_2$, the rate of consumption of $X_1$ is twice the rate of consumption $X_2$. 
##
Another more automaticic way of doing it is to use the matrix of stochoimetric coefficients $(B-A)'$. Therefore:
...
md"## 8.2 General formulation of the Mass Action Law

When we have to write the differenetial equations of more complex systems of interactions. To do this, we will use a general method based on the following equation: 
```math
\begin{align}
...
again,  `A` and `B` are the matrices with the stoichiometric coefficients, `X` is the state vector $X=[X_1,X_2,X_3]^T$. 
##
Now `K` is a matrix in the form:
...
```
## 
and $X^A$ is a column vector, calculated as:
...
md" ## 8.3 Example of differential form of Mass Action Law
...
i.e., a system of two reactions $r=2$ and four species $s=4$, with the stoichiometric matrices `A` and `B` listed in Eq. 30"

# ╔═╡ 6c491ea3-10b9-46b3-a3ff-2c27d029efe0
begin
	A3=[1 1 0 0; 0 0 1 2]
	B3=[0 0 1 2; 1 1 0 0]
	stoichiometric_matrix= transpose(B3-A3)
end
...
```
##
In other words, the differential equations are obtained by multiplying the matrix $(B-A)'$ by a vector of speeds of the reactions involved. 
##
This way, the final set of equations is:
...
# ╔═╡ 2c093ace-a53b-4fd8-91f9-3816e4ccdf62
md" ## 
> __Exercise 1__: Find the order of the reactions, the units of the kinetic rate constants, and write the differential equations that govern the dynamics of the following system:
>```math
...
function Calculate_Keq_stoichiometric_generic(A,B)
 ...
 for i=1:size(A)[1]
    	md"The order of the reaction $i is $(sum(A[i,:]))"
    #println("The Units of k$i are s^-1 . k^$(1-(sum(A[i,:])))")
 end
    stoichiometric_matrix= transpose(B-A)

	 md"The stoichiometric matrix is $stoichiometric_matrix" 	
...
md"## 
The $K$ matrix and the $X^A$ vector required for the equation for the general form of the Mass action are:
...
# ╔═╡ 9eff9a4e-4fcb-41a2-9924-0bf56de12eb7
md"
##
> __Task 2__: Find the order of the reactions, the units of the kinetic rate constants, and write the differential equations that govern the dynamics of the following system:
>
>```math 
...
# ╔═╡ 49eca337-d523-49c7-bc16-973f3a6a5638
md"
##
> __Task 3:__  Find the order of the reactions, the units of the kinetic rate constants, and write the differential equations that govern the dynamics of the following system:
>
>```math 
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "42b8044ec1e8e581f06a777e9579d4de6070d012"
