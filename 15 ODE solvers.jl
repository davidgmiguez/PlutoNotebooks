...
using PlutoUI,Plots , DifferentialEquations,ParameterizedFunctions

# ╔═╡ 2c2197b9-caec-4581-8f9f-a9b61974ed64
html"<button onclick='present()'>present</button>"

# ╔═╡ c981ee7c-93e1-4fe2-a337-9920d27a2612
begin
	struct Foldable{C}
	    title::String
	    content::C
...
md"##
Notice here we used the in-place format which writes the output to the preallocated vector `du`. For systems of equations the in-place format is faster. We define a vector with the initial concentrations of the variables `u₀`. For our model, the only parameter is the rate constant `k`, therefore we build the parameter collection `p` as"
...
md"##
Next, we define the `tspan` vector, which now it has only two components: the initial and final time point for the simulation"
...
md"##
In Julia, we need to define an __ODEProblem__ type using the constructor call. This is done by specifying this function __simpleODE__, the initial condition `u₀`, the time span `tspan` and the parameters  `p`:"
...
md"##
The solution acts like a matrix where `sol[j,i]` is the value of the `j`th variable at time `i`:"

# ╔═╡ 981c2e93-9495-4482-9c5c-970c6c8e03d2
sol1[:,3]

# ╔═╡ 9dba0d1c-1ede-4fa0-a241-a7ef29538006
md" ## 
We can make a plot of the results like this"
...
# ╔═╡ 78335b4f-2aba-4d20-8cc6-028e80b98afc
md"##"
...
# ╔═╡ f6c44c50-ca3b-4791-ac24-e4bc22033ebb
md"##"
...
with rate constants `k1`, and `k2` for the forward and reverse reaction, and with initial concentrations $X_1(0)$, $X_2(0)$ and $X_3(0)$. 

##
The stoichometric matrices are
...
md"##
One of the things that we can do when we have the differential equations of a system is to find the values at equilibrium. Since we already know that the equilibrium is reached when the speed of the reaction is zero, we have 
...
```
at any time, the condition $[a]+[c]=[a_o]$ is true at all time points. 

##
Thererefore we can rewrite the equilibrium equation as:
...
# ╔═╡ 5e3e039b-93e2-4b6d-949c-69400ea67c65
md"
##
>__Task 7:__  Solve numerically the  systems of interactions developed as part of task 5 and 6 in the last chapter. Choose values for kinetic constants and initial conditions that show dynamics in the firts 10 seconds of reaction. 
>- Compare the output of the two systems, and discuss the main differences. 
>- Introduce 10% variability in the initial conditions and run 50 simulations, calculate the average dynamics and plot the 95% confidence interval.
"

# ╔═╡ fd05bf43-77db-482d-ae1e-2a2d10b60d64
md"
##
>__Task 2:__  Solve numerically the following reversible reaction using a solver from the programming language of your preference (you may need to write your code as separate  functions). Use the Mass Conservation Law to simplify the system (only three ODEs).
> ```math
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "e6325be7c23d79235df6ad12183f27e889059dee"
