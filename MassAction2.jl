...
# ╔═╡ 2dd1c24e-88ef-11ec-1e8b-03e2932059a6
...
# ╔═╡ 57fec2e7-bc22-45df-b42a-58d466033abd
begin
	intial_a = @bind a₀ html"<input type=range min=0.01 max=0.1 step=0.01>"
	
	intial_b = @bind b₀ html"<input type=range min=0.01 max=0.1 step=0.01>"
	intial_c = @bind c₀ html"<input type=range min=0.01 max=0.1 step=0.01>"
	intial_d = @bind d₀ html"<input type=range min=0.01 max=0.1 step=0.01>"
...
# ╔═╡ 15fae3e2-aad3-4753-bd6d-1a62d0820d4c
...
# ╔═╡ ad8db90e-d40f-43b4-816e-2704a0dcd866
begin
	k1=1.3e0;  # units 1/(Ms)
	...
	p1=plot(sol1,xlabel="Time [s]",ylabel="Concentration [M]",title="Dynamics",ylims = (0,0.2));

	Q1=(sol1[3,:].*sol1[4,:].^2)./(sol1[1,:].*sol1[2,:])
	p2=plot(sol1.t,Q1,title= "Quotient coefficient",xlabel="Time [s]",ylabel="Q [M]",ylims = (0,0.6))
...
currentScript.previousElementSibling.innerText = "Computer Task 2: B-A'"
...
# ╔═╡ cbec5483-9fd4-40d5-b11c-12d9a0ad01de
md" Write a function to take as input the matrices A and B corresponding to the reaction 30 and returns the value $(B-A)'$ "
